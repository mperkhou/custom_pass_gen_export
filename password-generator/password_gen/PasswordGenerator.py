...
class PasswordGenerator:
    ...
    def _validate_consecutive(self, password, max_allowed_consecutive): #validate consecutive
        count = 1
        for i in range(1, len(password)):
            if password[i] == password[i-1]:
                count += 1
                if count >= max_allowed_consecutive:
                    print(f"{password[i]} occured more than {max_allowed_consecutive} times consecutively")
                    return False
            else:
                count = 1
        return True
    ...
    def _allowed(self,password: str) -> bool: #checks everything but length.
        ...
        if password in banned_words: # Check verboten
...
FILENAME = "C:\CPSE-coding-challenge-2022.01\config.json"
rules = getRules(FILENAME)

newPassword = PasswordGenerator(rules)
new_password = newPassword.allowed('tyr1piiltfWag7')